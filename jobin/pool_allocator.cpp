...
memory_chunk *pool_allocator::allocate_block(unsigned int n_blocks) {
  ...
  for (int i = 1; i < n_blocks; i++) {
    chunk->next = reinterpret_cast<memory_chunk *>(
        reinterpret_cast<char *>(chunk) + block_size);
    chunk = chunk->next;
  }
...
pool_allocator::pool_allocator(unsigned int bsize,
                               unsigned int pre_alocated_blocks) {
